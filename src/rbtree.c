...
#ifdef DEBUG
    if(!y->right)
        printf("Left rotation error! %d node right can't be NULL", node->data);
#endif
...
#ifdef DEBUG
    if(!y->left)
        printf("Right rotation error! %d node right can't be NULL", node->data);
#endif
...
static inline void irb_InsertFix(rbTree *tree, rbNode *node)
{
    ...
    while(node != *tree && node->parent->color == RED)
    {
        ...
        if(!grandParent)
        {
            DEBUG_PRINT("Missing grandparent of node %d", node->data);
            continue;
        } 
        #endif
        //left child
